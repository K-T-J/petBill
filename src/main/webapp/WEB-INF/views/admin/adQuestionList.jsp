...
<head>
  ...
  <!-- ================================================== 공통 메뉴 구분선 ========================================================== -->              
           ...
           <br />
           <div align="center">
           <form action="/petBill/admin/adQuestionList.pet">
           		<select name = "sel">
           			<option value="mtmuserid">작성자</option>
           			<option value="mtmsubject">제목</option>
           		</select>
					...
					<tr>
						...
						<td>
							<c:if test="${sel == null || search == null }">
								<a href="/petBill/admin/adQuestionContent.pet?mtmno=${article.mtmno}&pageNum=${pageNum}" class="nav-link">${article.mtmsubject}</a>
							</c:if>
							<c:if test="${sel != null || search != null }">
								<a href="/petBill/admin/adQuestionContent.pet?mtmno=${article.mtmno}&pageNum=${pageNum}&sel=${sel}&search=${search}" class="nav-link">${article.mtmsubject}</a>
							</c:if>
						</td>
						<td align="center">
			...
			<c:if test="${count > 0}">
				...
				<%--검색했을때 2페이지만 나오면 되는데 3페이지까지 나오게 됐을경우
				끝번호가 총페이지수 보다 많으면 끝번호를 총페이지번호로 덮어씌우기 --%>
				...
				<c:if test="${sel != null && search != null}">
					<c:if test="${startPage > pageBlock}">
						<a href="/petBill/admin/adQuestionList.pet?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}" class="page-link"> &lt;</a>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
						<a href="/petBill/admin/adQuestionList.pet?pageNum=${i}&sel=${sel}&search=${search}" class="page-link" >${i}</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<a href="/perBill/admin/adQuestionList.pet?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}" class="page-link"> &gt; </a>
					</c:if>
				</c:if>
				...
				<c:if test="${sel == null || search == null}">
					<c:if test="${startPage > pageBlock}">
						<a href="/petBill/admin/adQuestionList.pet?pageNum=${startPage-pageBlock}" class="page-link"> &lt;</a>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
						<a href="/petBill/admin/adQuestionList.pet?pageNum=${i}" class="page-link">${i}</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<a href="/petBill/admin/adQuestionList.pet?pageNum=${startPage+pageBlock}" class="page-link"> &gt; </a>
					</c:if>
