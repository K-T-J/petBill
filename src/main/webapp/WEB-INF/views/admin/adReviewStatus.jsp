...
#reviewContent{
  ...
  <!-- ================================================== 공통 메뉴 구분선 ========================================================== -->              
           ...
           <!-- 페이지 내용작성 영역 : Page content-->
            ...
            <br />
            <div align="center">
            <form action="/petBill/admin/adReviewStatus.pet">
           		<select name = "sel">
					...
					<tr>
						...
						<td>
							<c:if test="${sel == null || search == null }">
								<a href="/petBill/review/reviewModifyForm.pet?reviewNo=${article.reviewNo}&pageNum=${pageNum}" class="nav-link">${article.reviewSubject}</a>
							</c:if>
							<c:if test="${sel != null || search != null }">
								<a href="/petBill/review/reviewModifyForm.pet?reviewNo=${article.reviewNo}&pageNum=${pageNum}&sel=${sel}&search=${search}" class="nav-link">${article.reviewSubject}</a>
							</c:if>
						</td>
						<td>
							<input type="text" class="form-control-plaintext" readonly id="reviewContent" value="${article.reviewContent}"/>
						</td>
						<td>
							${article.hosname}
						</td>
			...
			<c:if test="${count > 0}">
				...
				<c:if test="${sel != null && search != null}">
					<c:if test="${startPage > pageBlock}">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${startPage-pageBlock}&sel=${sel}&search=${search}" class="page-link"> &lt;</a>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${i}&sel=${sel}&search=${search}" class="page-link">${i}</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${startPage+pageBlock}&sel=${sel}&search=${search}" class="page-link"> &gt; </a>
					</c:if>
				...
				<c:if test="${sel == null || search == null}">
					<c:if test="${startPage > pageBlock}">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${startPage-pageBlock}" class="page-link"> &lt;</a>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${i}" class="page-link">${i}</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<a href="/petBill/admin/adReviewStatus.pet?pageNum=${startPage+pageBlock}" class="page-link"> &gt; </a>
					</c:if>
